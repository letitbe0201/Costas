...
if( $ix=~ m/DW0/ ) {} else {
  ...
  printf sf "set_clock_uncertainty -hold 0.55 [ get_clocks clk ]
...
set_driving_cell -lib_cell CND2X1 [remove_from_collection [all_inputs] [get_port clk] ]
set_drive 0 clk
set_input_delay 0.5 -clock clk -clock_fall [remove_from_collection [all_inputs] [get_port clk] ]
set_output_delay 0.5 -clock clk -clock_fall [remove_from_collection [all_outputs] [get_port clk] ]
set_fix_hold [ get_clocks clk ]
set_max_fanout 100000 clk
...
printf sf "read_verilog %s\n",$vcomm;
printf sf "check_design\n";
printf sf "set_driving_cell -lib_cell CND2X1 [remove_from_collection [all_inputs] [get_port clk] ]
set_drive 0 clk
set_dont_touch_network clk\n";
printf sf "create_clock clk -name clk -period %f\n",$clkperiod*0.95;
printf sf "set_propagated_clock clk
set_clock_uncertainty -hold 0.55 [ get_clocks clk ]
set_clock_uncertainty -setup 0.25 [ get_clocks clk ]
set_propagated_clock clk
dont_touch tc240c/CDLY2XL
dont_use tc240c/CFDN2QXL
...
set_input_delay 0.5 -clock clk -clock_fall pushADC
set_input_delay 0.5 -clock clk -clock_fall ADC
set_output_delay 0.5 -clock clk -clock_fall pushByte
set_output_delay 0.5 -clock clk -clock_fall Byte
set_output_delay 0.5 -clock clk -clock_fall Sync
set_output_delay 0.5 -clock clk -clock_fall lastByte
set_input_delay 0.5 -clock clk -clock_fall stopIn
set_fix_hold [ get_clocks clk ]
set_max_fanout 100000 clk
set_dont_touch_network clk
compile -map_effort medium
